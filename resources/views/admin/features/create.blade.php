...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Add New Feature</h1>
                <p class="text-gray-600 mt-1">Create a new feature for Why Choose Us section</p>
            </div>
        ...
        <div class="p-6">
            <form action="{{ route('admin.features.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="icon" name="icon" value="{{ old('icon') }}" placeholder="e.g., fas fa-check-circle"
                ...
                <div>
                    ...
                    <select id="position" name="position" required
                        ...
                        <option value="left" {{ old('position') === 'left' ? 'selected' : '' }}>Left Column</option>
                        <option value="right" {{ old('position') === 'right' ? 'selected' : '' }}>Right Column</option>
                    </select>
                    @error('position')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
