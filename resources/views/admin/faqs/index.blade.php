...
<div class="container-fluid">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.faqs.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">Add FAQ</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow mb-6 p-4">
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-64">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search FAQs..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.faqs.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md transition-colors">Clear</a>
            </div>
        </form>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" name="selected_faqs[]" value="{{ $faq->id }}" class="faq-checkbox rounded">
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            <a href="{{ route('admin.faqs.edit', $faq) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
...
function deleteSelected() {
    const selected = document.querySelectorAll('.faq-checkbox:checked');
    if (selected.length === 0) {
        alert('Please select FAQs to delete');
        return;
    }
    
    if (confirm(`Are you sure you want to delete ${selected.length} selected FAQs?`)) {
        const ids = Array.from(selected).map(cb => cb.value);
        
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '{{ route("admin.faqs.bulk-delete") }}';
